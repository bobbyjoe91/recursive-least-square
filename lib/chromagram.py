...
def CQT_fast(x,fs,bins,fmin,fmax,M):
	...
	tempKernel = np.zeros(nfft, dtype = np.complex) # ==> cursive_K
	specKernel = np.zeros(nfft, dtype = np.complex) # ==> K
	sparKernel = []
	...
	for k in range(K-1,-1,-1):
		...
		specKernel[np.where(np.abs(specKernel) <= threshold)] = 0 # thresholding
...
def compute_chroma(x,fs):
	...
	bins = 12
	M = 3
	nOctave = np.int32(np.ceil(np.log2(fmax/fmin))) # 6 octaves
	CH = np.zeros(bins)
